...
export function RightPanel({ children, mobileOpen = false, onClose }: RightPanelProps) {
      ...
      <div className="h-9 px-3 border-b border-graphite-700/30 flex items-center justify-between shrink-0">
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h2 className="text-[9px] font-bold text-graphite-400 tracking-[0.15em] uppercase truncate">Dashboard</h2>
          </div>
        )}
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="text-graphite-500 hover:text-white hover:bg-white/10 transition-all duration-200 cursor-pointer shrink-0 w-5 h-5 flex items-center justify-center rounded ml-auto"
  ...
  return (
    <>
      <aside
        className={`shrink-0 bg-gradient-to-b from-graphite-900 to-graphite-950 border-l border-graphite-700/40 flex flex-col h-full transition-all duration-300 ${
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[1100] md:hidden">
          ...
          <aside className="absolute right-0 w-80 max-w-[85vw] h-full bg-gradient-to-b from-graphite-900 to-graphite-950 border-l border-graphite-700/40 flex flex-col shadow-2xl animate-[slide-in-right_0.2s_ease-out]">
            <div className="flex items-center justify-between h-9 px-3 border-b border-graphite-700/30 shrink-0">
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-md bg-accent/10 flex items-center justify-center">
                  <IconActivity className="w-3 h-3 text-accent" />
                </div>
                <h2 className="text-[9px] font-bold text-graphite-400 tracking-[0.15em] uppercase">Dashboard</h2>
              </div>
              <button
                onClick={onClose}
